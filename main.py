...
- hold layer down => switch to layerChangeLayer
- print combos
- print current layer
- mouse layer
...
from Utils.HIDKeyboard import Keyboard
from Utils.HIDKeyboard.keyboard_keys import *
...
from Utils.layers import (
    ...
    gamingLayerNumber, layerChangeLayerNumber, maxLayer
)
...
import Utils.mouseManager as mouseManager
...
oneHandedMode = False # one handed mode for mod keys

tempChangeLayerNumber = 5 # temp layer change to this layer
fallBackLayerNumber = 1  # after temp layer change, fall back to this layer
...
def GetActiveKey(row, col, isLong):
    ...
    # return L4_SHORT_GAME_01[row][col]
    layer = LAYER_SHORT_MAP.get(gamingLayerNumber)
    LX_SHORT_GAME_01 = layer.get("layout")
    # LX_SHORT_GAME_01 = LAYER_SHORT_MAP.get(gamingLayerNumber)
    return LX_SHORT_GAME_01[row][col]
# --------------------------------------------------
# Core logic
# --------------------------------------------------

def HandleLayerXPress(key, layerNumber):
    ...
    layer = LAYER_SHORT_MAP.get(layerNumber)
    LX_SHORT = layer.get("layout")

    r, c = key
    keyName = LX_SHORT[r][c]

    # print(f"HandleLayerXPress keyName: {keyName}")

    if keyName.startswith('MOUSE_'):
        # print("inside long SPECIAL_") 
        
        print(f"in mouse keyName: {keyName}")

        # only check for mouse speed
        if keyName.startswith('MOUSE_SPEED_'):
            mouseSpeed = int(keyName.replace('MOUSE_SPEED_', ''))
            mouseManager.MouseSpeed(mouseSpeed)

        # only checks for mouse action
        else:
            mouseAction = keyName[len('MOUSE_'):].lower()
            print(f'True: {mouseAction}')
            mouseManager.Main(mouseAction, True)

        time.sleep(mouseLoopDelay)
        return

def HandleLayerXHold(key, layerNumber):
    global activeModifiers
    global isLayerTempLayerUp01Active
    global isLayerTempLayerUp02Active
    global layerChangeLayerNumber
    global isModifierHold
    global oneHandedMode
    global tempChangeLayerNumber
    global fallBackLayerNumber
    global currentLayer
    # print(f"HandleLayerXHold:")

    layer = LAYER_LONG_MAP.get(layerNumber)
    LX_LONG = layer.get("layout")
    ...
    if time.time() - pressTime[key] >= longPressTime:
        ...
        if keyName.startswith('FUNCTION_'):
            ...
            if keyName == 'ACTION_SHUTDOWN':
                PiShutdown()
            elif keyName == 'ACTION_REBOOT':
                PiReboot()
            ...
            if keyName == 'LAYER_UP':
                ...
                if keyName == 'LAYER_TEMP_LAYER_UP_01':
                    ...
                    changeLayerTo = tempChangeLayerNumber
                    fallBackLayerNumber = currentLayer
                    tempLayerUp = True
        ...
        elif keyName.startswith('ONE_HANDED_'):
            oneHandedMode = True
            getModKeyName = keyName.replace('ONE_HANDED_','')
            mod_val = keyMap[f"KEY_{getModKeyName}"]
            ...
            # print('inside special combos')
            # print(special_combo)
            if special_combo:
                special_feature = special_combo[0]
                # print(f"special_feature: {special_feature}")
                if special_feature == 'REPEAT':
                    repeat_count = special_combo[1]
                    # print(f"repeat_count: {repeat_count}")
                    for i in range(int(repeat_count)):
                        main_key = special_combo[2]
                ...
                print(f"[SEND] LONG SPECIAL COMBO {keyName}")
                longSent.add(key)
                return
...
def HandleLayerXRelease(key, layerNumber):
    global activeModifiers
    global keyMap
    global tempLayerUp
    global currentLayer
    ...
    # LX_SHORT = LAYER_SHORT_MAP.get(layerNumber)
    # print(f'HandleLayerXRelease layerNumber: {layerNumber}')
    ...
    if key in longSent:
        ...
        # print(f"print in release, in longsend isModifierHold: {isModifierHold}")
        # print(f"print in release, in longsend tempLayerUp: {tempLayerUp}")

        if tempLayerUp:
            print(f"[RELEASE] LONG LAYER_TEMP_LAYER_UP_xx")
            changeLayerTo = fallBackLayerNumber
            fallBackLayerNumber = 1 # after this variable used , change the value to default value
            SwitchLayer(delta=0, layerNumber=changeLayerTo)
            tempLayerUp = False
            isLayerTempLayerUp01Active = False
            isLayerTempLayerUp02Active = False
        ...
        if isModifierHold:
            ...
            # print(f"print in release, in longsend keyNameLong: {keyNameLong}")
    ...
    # ['','','','','','','LAYER_CHANGE_BASE','LAYER_CHANGE_GAMING','',''],
    ...
    elif keyName.startswith('MOUSE_'):
        # print("inside long SPECIAL_") 
        
        print(f"in mouse release keyName: {keyName}")
        ...
        # only checks for mouse action
            ...
            mouseAction = keyName[len('MOUSE_'):].lower()
            # print(mouseAction)
            print(f'False: {mouseAction}')
            # input("human")
            mouseManager.Main(mouseAction, False)
        
        time.sleep(mouseLoopDelay)
        return

    elif keyName.startswith('MEDIA_'):

        print(f"[SEND] SHORT {keyName}")
        Media.send_key(keyMap[f"KEY_{keyName}"])
        return

        # if keyName == 'MEDIA_PLAY_PAUSE':
        #     print('inside MEDIA_PLAY_PAUSE')
        #
        #     # Media.send_key(MEDIA_PLAY)
        #     Media.send_key(keyMap[f"KEY_{keyName}"])
        #
        #     return
    # SHORTCUT_SWITCH_WINDOW_TO_LEFT
    elif keyName.startswith('SHORTCUT_'):
        print(f"[INFO] SHORT {keyName}")
        combo_keys = SHORTCUT_COMBOS[keyName]
        # combo_keys = PRINT_COMBOS[keyName]
        current_modifiers = activeModifiers  # Preserve any existing modifiers
        ...
        for combo_key in combo_keys:
            if combo_key in modifierNames: #or combo_key == 'LEFT_SHIFT':
                ...
                mod_val = keyMap[f"KEY_{combo_key}"]
                current_modifiers |= mod_val
                print(f"[MOD] SHORTCUT_COMBOS adding {combo_key}")
            else:
                # Send non-modifier key WITH accumulated modifiers
                print(f"[SEND] SHORTCUT_COMBOS {combo_key} (mods={current_modifiers})")
                Keyboard.send_key(current_modifiers, keyMap[f"KEY_{combo_key}"])
                current_modifiers = 0  # Clear after sending
...
def PrintLayerShort(layer_num):
    layer = LAYER_SHORT_MAP.get(layer_num)
    ...
    layer = LAYER_LONG_MAP.get(layer_num)
...
def UpdateKeyboardGamingLayer():
    ...
    # print("UpdateKeyboardGamingLayer")
    global currentLayer
...
def MainLoop():
    ...
    while True:
        ...
        # ---------------- Normal Layers ----------------
        if (currentLayer >= 1 and currentLayer < gamingLayerNumber) or currentLayer == layerChangeLayerNumber:
            ...
            for key in now - pressed:
                ...
                HandleLayerXPress(key, currentLayer)

            for key in now:
                # HandleLayerXHold(key, L1_LONG)
                HandleLayerXHold(key, currentLayer)

            for key in pressed - now:
                # HandleLayerXRelease(key, L1_SHORT, L1_LONG)
                HandleLayerXRelease(key, currentLayer)
