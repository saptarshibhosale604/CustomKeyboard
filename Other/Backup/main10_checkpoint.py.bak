...
from piSystemActions import PiShutdown, PiReboot
...
def HandleLayerXHold(key, layerNumber):
    ...
    # layerShort = LAYER_SHORT_MAP.get(layerNumber)
    # LX_LONG = LAYER_SHORT_MAP.get(layerNumber)
    # LX_LONG = LAYER_LONG_MAP.get(layerNumber)
    layer = LAYER_LONG_MAP.get(layerNumber)
    LX_LONG = layer.get("layout")
    
    # print(f"LX_LONG: {LX_LONG} ::")
    # print(f"type LX_LONG: {type(LX_LONG)} ::")
...
def MainLoop():
    ...
    while True:
        ...
        # ---------------- Normal Layers ----------------
        if (currentLayer >= 1 and currentLayer <= 4) or currentLayer == layerChangeLayerNumber:
        ...
        #
        #     for key in now:
        #         HandleLayerXHold(key, L6_LONG)
        #
        #     for key in pressed - now:
        #         # HandleLayerXRelease(key, L5_SHORT)
        #         HandleLayerXRelease(key, L6_SHORT, L6_LONG)
        #
        #     pressed = now
        #     time.sleep(scanDelay)
        #
